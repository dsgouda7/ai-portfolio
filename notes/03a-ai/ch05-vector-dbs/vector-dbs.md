...
The 1.2-second retrieval pause identified in §0 has one root cause: brute-force search runs O(N) — every query touches every document chunk. At 200 documents it's invisible; at 50,000 documents it's the blocker that makes real-time RAG unusable.
...
**Investigation scale path:** At the current 200-document prototype, FAISS Flat (exact search) is the correct choice — the corpus fits in ~2 MB of RAM and retrieval costs <1ms. The switch to HNSW triggers when the corpus exceeds ~50K documents and retrieval latency breaches your SLA. Measure first; don't add index complexity before you hit the wall.

> 💡 **Exact search → scale wall:** At 200 documents, brute-force retrieval costs ~5ms and is unnoticeable. At 50,000 documents it costs ~1,200ms — triggering query timeouts and making real-time RAG unusable. The §0 scaling failure is simply the O(N) cost curve becoming visible.

***

## 2. Distance Metrics: The Foundation of Similarity Search

The wrong metric choice doesn't produce a latency error — it silently returns wrong documents. A metric mismatch could surface irrelevant chunks and degrade retrieval precision below the 97% recall@5 target. Choose before you index; changing metrics later requires a full re-embed.
...
**Investigation metric choice:** MiniLM text embeddings are not L2-normalised by default — cosine similarity is the correct choice. It measures semantic angle regardless of embedding magnitude, making short queries like "uptime target" and longer queries like "authentication service SLA requirements for production" compare fairly against stored wiki chunks.

> 💡 **Metric choice → recall:** Switching from cosine to dot product on un-normalised MiniLM embeddings can silently degrade recall by 5–15% on short queries — degrading retrieval quality without any visible error signal. Cosine is the safe default for text embeddings until you've verified your model normalises outputs.

***

## 3. Core ANN Indexing Techniques

§0 establishes two scaling failure modes: O(N) latency (1.5s pause at 50K documents) and O(N·d) memory (153.6MB for 50K uncompressed float32 vectors). The three index families here attack different parts of that problem — IVF and HNSW attack latency through smarter search paths; PQ attacks memory through compression. Pick the right one and the production rollout is viable; pick wrong and one failure mode survives.
...
> 💡 **IVF → investigation latency:** At 50K documents with nlist=1,024 and nprobe=8, IVF reduces the effective search from 50,000 comparisons to ~390 (50K/128). That drops retrieval from 1.5s to ~12ms — below the 100ms latency threshold. But adding new documents triggers a cluster rebuild. For frequently-updated document pipelines, this rebuild cost makes IVF operationally expensive compared to HNSW's O(M log N) per-insert.

***

### 3.2 HNSW — Hierarchical Navigable Small World

HNSW is the direct solution to §0's scaling failure: O(log N) graph traversal replaces O(N) brute-force, and real-time inserts mean new documents don't trigger a cluster rebuild. This is the index that makes the board's production rollout viable.
...
> 💡 **HNSW → production viability:** At 50K documents with M=16 and ef_search=64, HNSW delivers <10ms retrieval latency (vs. 1.5s brute-force) with >99% recall@5 — eliminating query timeouts from §0. New document inserts take ~5ms per item. This is the combination that earns board approval for the production rollout.
...
Map §0's scaling failure modes to the index that solves each: O(N) latency → HNSW or IVF; memory explosion → PQ or IVF-PQ; rebuild cost → HNSW or DiskANN. For the investigation's 50K-document corpus with frequent updates, read the HNSW row.
...
HNSW fixes the O(N) latency failure from §0. But real production queries — e.g., "authentication SLA for EU region services" — combine a metadata constraint (region), an exact keyword ("authentication"), and a semantic intent (service reliability policies). No single index handles all three signals optimally, which is where hybrid approaches earn their complexity cost.
...
The §0 prototype runs FAISS in-process — fast, zero-ops, single machine. The production rollout demands persistence, multi-tenancy, and managed failover — none of which FAISS provides. §7 maps the scale ladder from prototype library to production distributed system.
...
## 13 · Progress Check — AI Literacy Kit: Chapter 5 Findings

> 🔬 **Board deliverable:** The Staff Engineer has completed the ANN index benchmark. The findings are ready to present: "Yes, the RAG pipeline scales to production."

**Chapter 5 investigation summary:**

The core question was: *Can the wiki RAG pipeline (currently 200 docs) scale to 50,000 documents without exceeding latency or memory constraints?*

The answer: **Yes — with the right index.**

**Benchmark results at 50K documents:**

| Index | Latency | Recall@5 | Memory | Update Cost |
|-------|---------|----------|--------|-------------|
| Brute-force (baseline) | ~1,200ms | 100% | 153.6 MB | O(1) insert |
| IVF (nlist=1024, nprobe=8) | ~12ms | 97.2% | 153.6 MB | Cluster rebuild |
| **HNSW (M=16, ef=200)** | **~4ms** | **98.6%** | **~160 MB** | **O(M log N) insert** |
| HNSW + SQ8 | ~5ms | 98.1% | ~40 MB | O(M log N) insert |
| HNSW + PQ | ~5ms | 96.8% | ~5 MB | O(M log N) insert |

**Unlocked capabilities:**
- ✅ **HNSW index**: Sublinear O(log N) search replaces O(N) brute-force — 300× faster at 50K docs
- ✅ **Scalar quantization (SQ8)**: 4× memory reduction with ≤1% recall loss
- ✅ **Product quantization (PQ)**: 32× memory reduction — 153.6MB → 4.8MB for 50K chunks
- ✅ **Metadata filtering**: Region, document-type, date-range filters without full corpus scan
- ✅ **Hybrid retrieval**: BM25 + HNSW with RRF fusion for exact-term + semantic queries

**Progress toward investigation constraints:**

| Constraint | Status | Evidence |
|------------|--------|---------|
| Retrieval latency <100ms | ✅ **MET** | HNSW: ~4ms at 50K docs (300× improvement) |
| Recall@5 ≥ 97% | ✅ **MET** | HNSW: 98.6% recall@5 |
| Memory ≤ server RAM | ✅ **MET** | HNSW+SQ8: ~40MB for 50K docs |
| Supports weekly doc additions | ✅ **MET** | HNSW dynamic inserts: ~5ms/doc, no rebuild |

**What the board needs to hear:**

The scaling analysis validates the production rollout:

1. **Latency is solved**: 4ms HNSW retrieval vs. 1,200ms brute-force. Query timeouts eliminated.
2. **Memory is manageable**: SQ8 compression holds the 50K-document index under 40MB — single commodity server.
3. **Update frequency is handled**: HNSW accepts new documents in milliseconds. The 200 docs/week ingestion rate costs <1 second of insert time total.
4. **Recall is preserved**: 98.6% recall@5 — the 1.4% trade-off is an accepted engineering compromise for a 300× latency improvement.

**Recommendation:** Deploy HNSW with SQ8 compression. Index takes ~30s to build at 50K documents. Set `ef_search=64` for the latency/recall operating point shown in the benchmark. Re-evaluate at 500K documents — DiskANN becomes relevant when the corpus exceeds available RAM.

**Next chapter**: [ReAct & Semantic Kernel](../ch06-react-and-semantic-kernel) adds orchestration for multi-step query decomposition — enabling complex queries like "compare authentication SLA across EU and US regions" that require multiple retrieval + reasoning steps.
...
Vector DBs (Ch.5) solved the *infrastructure* half — fast retrieval at production scale. Ch.6 adds the orchestration layer that enables multi-step reasoning over that retrieved context.
