"""
gen_ch03.py — Generate visual assets for Ch.3 Matrix Factorization
Produces:
  img/ch03-matrix-factorization-needle.gif        — HR@10 needle:
        item-CF 65% → plain MF 75% → bias MF 78%
  img/ch03-matrix-factorization-progress-check.png — FlixAI constraint dashboard
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. NEEDLE GIF  — HR@10 gauge (higher = better)
# ─────────────────────────────────────────────────────────────────────────────
...
def _needle_frame(ax, hr_value: float, phase: str, frame_idx: int, n_frames: int):
    ...
    # Gauge arc (180° semicircle, 0 on left, 100% on right)
    hr_min, hr_max = 0.0, 100.0

    # Zone boundaries (HR@10 %)
    zone_bounds = [0, 42, 65, 78, 85, 100]
    zone_colors = [DANGER, CAUTION, INFO, CAUTION, SUCCESS]
    zone_labels = ["0%", "42%\n(pop.)", "65%\n(CF)", "78%\n(bias MF)", "85%\n(target)", "100%"]
    ...
    for tb, tl in zip(zone_bounds, zone_labels):
        ...
        ax.text(xl, yl, tl, ha="center", va="center", color=FG,
                fontsize=5, zorder=4, multialignment="center")
    ...
    # Centre readout
    ax.text(0, 0.22, f"HR@10", ha="center", va="center",
            color=MUTED, fontsize=7, zorder=5)
    ax.text(0, 0.10, f"{hr_value:.0f}%", ha="center", va="center",
            color=GOLD, fontsize=15, fontweight="bold", zorder=5)
...
def make_needle_gif():
    """Animate HR@10 rising from item-CF baseline through MF improvements."""
    # Keyframes: (hr_value_%, phase_label, hold_frames)
    keyframes = [
        (65.0,  "Item-CF baseline (~65%)",        5),
        (67.0,  "Init latent factors P, Q...",    1),
        (69.0,  "SGD epoch 1 — large gradients",  1),
        (71.0,  "SGD converging...",              1),
        (73.0,  "Latent space forming",           1),
        (75.0,  "Plain MF (k=50, λ=0.05) 75%",   5),
        (76.5,  "Adding user/item bias terms",    1),
        (78.0,  "Bias MF — HR@10 = 78%",          7),
    ]
    ...
    out_path = OUT_DIR / "ch03-matrix-factorization-needle.gif"
    pil_frames[0].save(
        out_path,
        save_all=True,
...
def make_progress_check():
    ...
    fig.suptitle("Ch.3 — Matrix Factorization: Progress Check",
    ...
    milestones = [
        ("Popularity baseline\n(Ch.1)",        42,  DANGER),
        ("Item-CF\n(Ch.2)",                    65,  CAUTION),
        ("Plain MF  k=50\n(this chapter)",     75,  INFO),
        ("Bias MF  μ+bᵤ+bᵢ+pᵤᵀqᵢ\n(this chapter)", 78, INFO),
    ...
    ax.text(85.5, len(labels) - 0.4, ">85%\ntarget",
            color=SUCCESS, fontsize=7, va="top")

    # "You are here" arrow
    ax.annotate("← Ch.3 result", xy=(78, 3), xytext=(70, 3.6),
    ...
    # Gap callout
    ax.annotate("", xy=(85, 0.5), xytext=(78, 0.5),
    ...
    ax2.text(0.5, 0.97, "FlixAI Constraints — Ch.3 Status",
             ha="center", va="top", color=FG, fontsize=10,
             fontweight="bold", transform=ax2.transAxes)

    constraints = [
        ("#1 ACCURACY",        ">85% HR@10",
         "WIP", "Bias MF → 78%\n7 pts to go (Ch.4 closes it)",    CAUTION),
        ("#2 COLD START",      "New users/items",
         "NO",  "No trained p vector\nfor new arrivals (Ch.6)",    DANGER),
        ("#3 SCALABILITY",     "<200ms, 1M+ ratings",
         "OK",  "O(k) predict; 21× less\nmemory vs item-CF",       SUCCESS),
        ("#4 DIVERSITY",       "Long-tail content",
         "WIP", "Latent space helps;\npopularity bias persists",   CAUTION),
        ("#5 EXPLAINABILITY",  "\"Because you liked X\"",
         "NO",  "Latent dims discovered,\nnot labelled",            DANGER),
    ]
    ...
    ax2.text(
        0.5, 0.135,
        "Model: R ≈ P·Qᵀ  |  k=50 factors  |  λ=0.05  |  bias: μ + bᵤ + bᵢ + pᵤᵀqᵢ",
    ...
    out_path = OUT_DIR / "ch03-matrix-factorization-progress-check.png"
...
if __name__ == "__main__":
    print("Generating Ch.3 Matrix Factorization visuals...")
    make_needle_gif()
    make_progress_check()
    print("Done.")
