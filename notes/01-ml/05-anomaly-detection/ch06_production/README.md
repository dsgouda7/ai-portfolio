...
> **This model is a planning tool, not a physical law.** Real drift rates depend on fraud ecosystem dynamics. The key insight is the shape: recall degrades fastest in the first weeks, then the rate slows as the remaining overlap between training and current fraud is more stable.
