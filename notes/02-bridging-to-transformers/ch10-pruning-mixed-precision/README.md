...
**Exercise 3 — The AMP bridge.** Open the MiniLM training loop in `learning/genai/02-transformers/02-decoder-only-language-model.ipynb` and wrap it in `tf.keras.mixed_precision`. Measure wall-clock time per epoch before and after. Predict: on a CPU (no Tensor Cores), will AMP still give 2× speedup? Why or why not?
