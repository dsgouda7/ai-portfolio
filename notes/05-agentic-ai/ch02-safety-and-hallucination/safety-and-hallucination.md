...
>
> **Where you are in the curriculum.** A system that is right 95% of the time is *dangerous* if users can't tell which 5% is wrong. This document covers the failure modes that production AI systems encounter — hallucination, prompt injection, jailbreaks, alignment drift — how to detect them, and the layered mitigation stack from prompt-level guards through application-level validators to model-level fine-tuning ([Ch.5 Fine-Tuning](../ch05-fine-tuning)) and evaluation ([Ch.3 Evaluating AI Systems](../ch03-evaluating-ai-systems)).
>
...
- **Open-domain** → Retrieval-Augmented Generation ([RAG](../ch03-rag-and-embeddings)) *converts* open-domain queries into closed-domain ones — retrieve the ground truth first, then generate against it. If retrieval fails, fail-safe to "I don't have that information."

> **Warning — PizzaBot fail-safe pattern:** A customer asking "What toppings are on the Margherita?" has a closed-domain answer in the menu corpus — retrieve and verify. A customer asking "What's in Mamma Rosa's secret sauce?" has no corpus chunk — the model is in open-domain territory and will confabulate. Detect this case by checking whether retrieval returned any relevant chunk (e.g., similarity score below threshold). If not, return "I don't have that information" rather than generate. A refusal is always cheaper than a wrong answer that reaches production.
...
| **Neutral** | Premise doesn't confirm or deny hypothesis | Unverifiable — treat as ungrounded (reject or flag) |
...
for claim in claims:
 ...
 verdict = " UNVERIFIABLE — flag for review"
 elif label == "ENTAILMENT" and score > 0.8:
 verdict = " GROUNDED — safe to return"
 else:
 verdict = " LOW CONFIDENCE — retry with stricter prompt"
...
if len(alerts) > 0:
 print(f" ALERT: Hallucination rate exceeded {threshold:.1%} on {len(alerts)} days:")
...
for prompt in test_prompts:
 ...
 action = " FLAG — allow but log for review"
 else:
...
def check_content_safety(text: str) -> dict:
 ...
 verdict = " FLAGGED — medium severity, log for review"
...
test_inputs = [
 ...
 "Your pizza sucks and I hate you", # profanity + mild hate
...
if results["prevention_rate"] < 0.95:
 print(f"\n ALERT: Attack prevention rate {results['prevention_rate']:.1%} < 95% threshold")
