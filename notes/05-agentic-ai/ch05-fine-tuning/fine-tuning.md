...
def prepare_fine_tuning_dataset(raw_conversations, target_format="alpaca"):
 ...
 if dedup_rate < 0.95:
 print(" WARNING: Low uniqueness (<95%) → risk of overfitting to repeated examples")
 ...
 if median_length < 50:
 print(" WARNING: Median length <50 tokens → examples may lack context")
 if median_length > 300:
 print(" WARNING: Median length >300 tokens → risk of overfitting to long examples")
 ...
 print(" WARNING: <10% negative examples → model may not learn what to avoid")
 ...
 print(f" WARNING: >{format_errors} format errors (>5%) → check data pipeline")
 ...
 print(" WARNING: <100 training examples → high variance, consider collecting more")
 if len(train_data) > 10000:
 print(" NOTE: >10k training examples → diminishing returns, focus on quality")
...
| **Eval loss final** | 1.589 | Rising after step 50 → overfitting begins |
...
> **Warning — Common mistake:** Skipping LoRA and going straight to DPO. DPO refines a model that already behaves approximately correctly — it needs a good SFT base to work from. Without LoRA first, the reference model ($\pi_{ref}$) is too far from the desired output distribution for DPO to converge reliably.
