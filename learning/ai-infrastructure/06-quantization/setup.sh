...
# Jupyter kernel named "quantization" pointing at that venv. The notebook's
...
if [ "$SKIP_KERNEL" -eq 0 ]; then
    echo "Registering Jupyter kernel 'quantization'..."
    "$VENV_PYTHON" -m ipykernel install --user --name quantization --display-name "Python (quantization .venv)"
fi

echo ""
echo "Setup complete."
echo "Open quantization-in-depth.ipynb and pick the 'Python (quantization .venv)' kernel"
