...
.DESCRIPTION
    ...
    Jupyter kernel named "quantization" pointing at that venv. Select the
    "Python (quantization .venv)" kernel in VS Code to use it.
...
if (-not $SkipKernel) {
    Write-Host "Registering Jupyter kernel 'quantization'..."
    & $VenvPython -m ipykernel install --user --name quantization --display-name "Python (quantization .venv)"
}

Write-Host "`nSetup complete." -ForegroundColor Green
Write-Host "Open quantization-in-depth.ipynb and pick the 'Python (quantization .venv)' kernel."
