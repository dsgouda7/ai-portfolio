...
# Jupyter kernel named "inference-systems" pointing at that venv. The notebook's
...
if [ "$SKIP_KERNEL" -eq 0 ]; then
    echo "Registering Jupyter kernel 'inference-systems'..."
    "$VENV_PYTHON" -m ipykernel install --user --name inference-systems --display-name "Python (inference-systems .venv)"
fi

echo ""
echo "Setup complete."
echo "Open inference-systems.ipynb and pick the 'Python (inference-systems .venv)' kernel"
