...
.DESCRIPTION
    ...
    Jupyter kernel named "inference-systems" pointing at that venv. Select the
    "Python (inference-systems .venv)" kernel in VS Code to use it.
...
if (-not $SkipKernel) {
    Write-Host "Registering Jupyter kernel 'inference-systems'..."
    & $VenvPython -m ipykernel install --user --name inference-systems --display-name "Python (inference-systems .venv)"
}

Write-Host "`nSetup complete." -ForegroundColor Green
Write-Host "Open inference-systems.ipynb and pick the 'Python (inference-systems .venv)' kernel."
