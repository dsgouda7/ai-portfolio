...
# Jupyter kernel named "profiling" pointing at that venv. The notebook's
...
if [ "$SKIP_KERNEL" -eq 0 ]; then
    echo "Registering Jupyter kernel 'profiling'..."
    "$VENV_PYTHON" -m ipykernel install --user --name profiling --display-name "Python (profiling .venv)"
fi

echo ""
echo "Setup complete."
echo "Open pytorch-profiling.ipynb and pick the 'Python (profiling .venv)' kernel"
