...
.DESCRIPTION
    ...
    Jupyter kernel named "profiling" pointing at that venv. Select the
    "Python (profiling .venv)" kernel in VS Code to use it.
...
if (-not $SkipKernel) {
    Write-Host "Registering Jupyter kernel 'profiling'..."
    & $VenvPython -m ipykernel install --user --name profiling --display-name "Python (profiling .venv)"
}

Write-Host "`nSetup complete." -ForegroundColor Green
Write-Host "Open pytorch-profiling.ipynb and pick the 'Python (profiling .venv)' kernel."
