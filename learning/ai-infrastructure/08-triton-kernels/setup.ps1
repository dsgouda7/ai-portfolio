...
.DESCRIPTION
    ...
    Jupyter kernel named "triton-kernels" pointing at that venv. Select the
    "Python (triton-kernels .venv)" kernel in VS Code to use it.
...
if (-not $SkipKernel) {
    Write-Host "Registering Jupyter kernel 'triton-kernels'..."
    & $VenvPython -m ipykernel install --user --name triton-kernels --display-name "Python (triton-kernels .venv)"
}

Write-Host "`nSetup complete." -ForegroundColor Green
Write-Host "Open triton-kernels.ipynb and pick the 'Python (triton-kernels .venv)' kernel."
