...
# Jupyter kernel named "gpu-hardware" pointing at that venv. The notebook's
...
if [ "$SKIP_KERNEL" -eq 0 ]; then
    echo "Registering Jupyter kernel 'gpu-hardware'..."
    "$VENV_PYTHON" -m ipykernel install --user --name gpu-hardware --display-name "Python (gpu-hardware .venv)"
fi

echo ""
echo "Setup complete."
echo "Open gpu-hardware-foundations.ipynb and pick the 'Python (gpu-hardware .venv)' kernel"
