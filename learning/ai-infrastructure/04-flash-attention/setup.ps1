...
.DESCRIPTION
    ...
    Jupyter kernel named "flash-attention" pointing at that venv. Select the
    "Python (flash-attention .venv)" kernel in VS Code to use it.
...
if (-not $SkipKernel) {
    Write-Host "Registering Jupyter kernel 'flash-attention'..."
    & $VenvPython -m ipykernel install --user --name flash-attention --display-name "Python (flash-attention .venv)"
}

Write-Host "`nSetup complete." -ForegroundColor Green
Write-Host "Open flash-attention-internals.ipynb and pick the 'Python (flash-attention .venv)' kernel."
