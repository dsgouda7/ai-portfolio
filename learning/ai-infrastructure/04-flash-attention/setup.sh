...
# Jupyter kernel named "flash-attention" pointing at that venv. The notebook's
...
if [ "$SKIP_KERNEL" -eq 0 ]; then
    echo "Registering Jupyter kernel 'flash-attention'..."
    "$VENV_PYTHON" -m ipykernel install --user --name flash-attention --display-name "Python (flash-attention .venv)"
fi

echo ""
echo "Setup complete."
echo "Open flash-attention-internals.ipynb and pick the 'Python (flash-attention .venv)' kernel"
