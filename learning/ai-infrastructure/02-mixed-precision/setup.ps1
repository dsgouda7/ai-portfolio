...
.DESCRIPTION
    ...
    Jupyter kernel named "mixed-precision" pointing at that venv. Select the
    "Python (mixed-precision .venv)" kernel in VS Code to use it.
...
if (-not $SkipKernel) {
    Write-Host "Registering Jupyter kernel 'mixed-precision'..."
    & $VenvPython -m ipykernel install --user --name mixed-precision --display-name "Python (mixed-precision .venv)"
}

Write-Host "`nSetup complete." -ForegroundColor Green
Write-Host "Open mixed-precision-and-memory.ipynb and pick the 'Python (mixed-precision .venv)' kernel."
