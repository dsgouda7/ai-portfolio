...
if [ "$SKIP_KERNEL" -eq 0 ]; then
    echo "Registering Jupyter kernel 'mixed-precision'..."
    "$VENV_PYTHON" -m ipykernel install --user --name mixed-precision --display-name "Python (mixed-precision .venv)"
fi

echo ""
echo "Setup complete."
echo "Open mixed-precision-and-memory.ipynb and pick the 'Python (mixed-precision .venv)' kernel."
