...
- **Recurring emoji callouts** — reuse these exact markers, they are load-bearing
  ...
  > **Plain-text equivalents are acceptable.** The mechanistic notebooks (`01-transformers/`,
  > `02-llm-finetuning/`) consistently use plain-text `#### Predict first` / `#### Your turn — ...`
...
## 8 · Narrative & Business-Stakes Framing (from the `02-llm-finetuning/` series)
...
tour of options. The `02-llm-finetuning/` 3-part series is the reference example -- a
...
## 9 · Code Walkthrough Cells (from `02-llm-finetuning/01-llm-finetuning-data-techniques.ipynb` iteration 2)
...
This section documents patterns extracted from a third pass over the `02-llm-finetuning/` series
series,
...
## 11 · Contrast Subsections — Introducing a Competing Technique Without Training It (from the DPO vs. PPO addition to `02-llm-finetuning/01-llm-finetuning-data-techniques.ipynb`)
...
subsection added to `02-llm-finetuning/01-llm-finetuning-data-techniques.ipynb`'s preference-alignment section is the reference
...
`02-llm-finetuning/02-llm-finetuning-parameter-techniques.ipynb`'s parameter-efficiency section -- illustrative
...
while drafting. The `02-llm-finetuning/` series is the reference example: "fine-tuning" isn't treated
...
bridge mechanics used consistently across `02-llm-finetuning/`, `03-rag/`, and `05-llm-gateway/`.
...
- For the first notebook in a chapter arc, the bridge maps from prior **chapters** (e.g.
  `01-transformers/`) rather than prior notebooks in the same folder.
