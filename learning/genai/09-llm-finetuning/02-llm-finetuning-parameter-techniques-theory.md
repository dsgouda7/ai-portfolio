...
LoRA keeps a projection's original weight frozen and learns a low-rank correction beside it:

$$
y = Wx + B(Ax).
$$

Here `x` is the layer input, `W` is the frozen base weight, `A` compresses the input into a small bottleneck, `B` expands it back to the output width, and `y` is the combined output. For a 4096-wide projection with rank 8, `W` has shape 4096 by 4096, while `A` is 8 by 4096 and `B` is 4096 by 8. The two narrow matrices hold far fewer trainable values than a full replacement for `W`.
