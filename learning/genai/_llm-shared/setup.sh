...
# the 04-llm-finetuning, 05-rag, and 06-llm-gateway notebooks.
#
...
if [ "$SKIP_KERNEL" -eq 0 ]; then
    echo "Registering Jupyter kernel 'genai-llm'..."
    "$VENV_PYTHON" -m ipykernel install --user --name genai-llm --display-name "Python (genai-llm .venv)"
fi

echo ""
echo "Setup complete."
echo "Open any fine-tuning, RAG, or gateway notebook and pick the 'Python (genai-llm .venv)' kernel"
