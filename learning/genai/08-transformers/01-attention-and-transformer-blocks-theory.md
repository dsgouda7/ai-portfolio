...
Here `Q`, `K`, and `V` are query, key, and value tensors; `d_k` is query/key width; and `M` is an optional mask. `QK^T` scores every query/key token pair. A blocked entry acts like negative infinity, giving it zero weight.
...
$$
a=x+\operatorname{MHA}(\operatorname{LN}_1(x)),\qquad
y=a+\operatorname{FFN}(\operatorname{LN}_2(a))
$$

Here `x` is the input, `a` is the post-attention state, and `y` is the output. `LN`, `MHA`, and `FFN` mean layer normalization, multi-head attention, and feed-forward network. Residuals give gradients a direct route through depth, as the notebook's 24-layer probe shows.
