...
**Reciprocal Rank Fusion (RRF)** ignores raw scores and rewards high positions across lists:

$$
\operatorname{RRF}(d)=\sum_{s:d\in s}\frac{1}{k+r_s(d)}
$$

Here $d$ is a document, $s$ is a retriever's ranked list, $r_s(d)$ is the rank of $d$ in that list, and $k$ is a damping constant that controls how quickly credit falls with rank. An absent document contributes nothing.
