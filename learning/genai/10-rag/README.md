...
Run `setup.ps1` on Windows or `setup.sh` on Linux/macOS; either script creates this chapter's `.venv`, installs `requirements.txt`, registers its Jupyter kernel, and assigns that kernel to both notebooks. The deeper general evaluation track remains in [`../11-llm-evaluation/`](../11-llm-evaluation/).
