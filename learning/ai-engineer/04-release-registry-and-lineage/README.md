...
Select that environment as the notebook kernel. Run the notebook from any working directory inside the checkout; its setup cell locates the repository root using `AUTHORING_GUIDE.md` and `learning/ai-engineer/shared`, then resolves all inputs from that root.
