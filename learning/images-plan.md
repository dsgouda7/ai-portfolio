...
| `genai/02-transformers/01-attention-and-transformer-blocks.ipynb` | Four strong component/flow visuals | No change |
| `genai/02-transformers/02-decoder-only-language-model.ipynb` | Autoregressive/KV-cache and scale bridge are strong | No change |
| `genai/02-transformers/03-encoder-decoder-and-cross-attention.ipynb` | Seven strong visuals; two mechanism gaps remain | Queue 9-10 |
| `genai/03-llm-finetuning/01-llm-finetuning-data-techniques.ipynb` | Four strong visuals/animations; DPO construction remains prose-heavy | Queue 11 |
| `genai/03-llm-finetuning/02-llm-finetuning-parameter-techniques.ipynb` | Spectrum, LoRA, and QLoRA visuals are strong | No change |
| `genai/03-llm-finetuning/03-llm-finetuning-comparison-and-decision.ipynb` | Executable charts and Mermaid decision flows are sufficient | No change |
| `genai/03-llm-finetuning/04-llm-finetuning-practice.ipynb` | Runtime dashboard is strong; long-run workflow needs a navigator | Queue 12 |
| `genai/04-rag/04-hybrid-search.ipynb` | Hybrid retrieval storyboard is strong | No change |
| `genai/04-rag/05-rag-evaluation.ipynb` | Failure-location map is strong; live charts cover the metrics | No change |
| `genai/05-llm-evaluation/01-llm-evaluation-metrics-and-benchmarks.ipynb` | Code-first charts are strong; taxonomy is hard to scan | Queue 13 |
| `genai/05-llm-evaluation/02-llm-as-judge-safety-and-pipeline.ipynb` | Generated bias/agreement/regression charts are sufficient | No Designer image; keep measured charts executable |
| `genai/05-llm-evaluation/03-hallucination-detection.ipynb` | Code-first metrics are strong; composed guard path is text-only | Queue 14 |
| `genai/05-llm-evaluation/04-calibration-and-confidence.ipynb` | Reliability/coverage plots are strong; decision sequence is text-only | Queue 15 |
| `genai/06-llm-gateway/06-llm-gateway.ipynb` | Lifecycle image is strong; routing and fallback dynamics need flow | Queue 16-17 |
...
- **Notebook:** `genai/02-transformers/03-encoder-decoder-and-cross-attention.ipynb`
...
- **Notebook:** `genai/03-llm-finetuning/01-llm-finetuning-data-techniques.ipynb`
...
- **Notebook:** `genai/03-llm-finetuning/04-llm-finetuning-practice.ipynb`
...
- **Notebook:** `genai/05-llm-evaluation/01-llm-evaluation-metrics-and-benchmarks.ipynb`
...
- **Target directory:** create `genai/05-llm-evaluation/images/` during integration
- **Aspect ratio:** 16:9
...
- **Notebook:** `genai/05-llm-evaluation/03-hallucination-detection.ipynb`
...
- **Notebook:** `genai/05-llm-evaluation/04-calibration-and-confidence.ipynb`
...
- **Notebook:** `genai/06-llm-gateway/06-llm-gateway.ipynb`
...
| Seven unreferenced `genai/03-llm-finetuning/images/*.png` assets | Review during Designer integration | Some may be older candidates; do not delete as a batch without opening each |
