...
# Jupyter kernel named "neural-networks" pointing at that venv. The notebook's
...
if [ "$SKIP_KERNEL" -eq 0 ]; then
    echo "Registering Jupyter kernel 'neural-networks'..."
    "$VENV_PYTHON" -m ipykernel install --user --name neural-networks --display-name "Python (neural-networks .venv)"
fi
