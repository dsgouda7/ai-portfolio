...
.DESCRIPTION
    ...
    Jupyter kernel named "neural-networks" pointing at that venv. Select the
    "Python (neural-networks .venv)" kernel in VS Code to use it.
...
if (-not $SkipKernel) {
    Write-Host "Registering Jupyter kernel 'neural-networks'..."
    & $VenvPython -m ipykernel install --user --name neural-networks --display-name "Python (neural-networks .venv)"
}
