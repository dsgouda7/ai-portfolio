...
.DESCRIPTION
    ...
    Jupyter kernel named "cnns" pointing at that venv. Select the
    "Python (cnns .venv)" kernel in VS Code to use it.
...
if (-not $SkipKernel) {
    Write-Host "Registering Jupyter kernel 'cnns'..."
    & $VenvPython -m ipykernel install --user --name cnns --display-name "Python (cnns .venv)"
}

Write-Host "`nSetup complete." -ForegroundColor Green
Write-Host "Open convolutional-neural-networks.ipynb and pick the 'Python (cnns .venv)' kernel."
