...
# Jupyter kernel named "cnns" pointing at that venv. The notebook's
...
if [ "$SKIP_KERNEL" -eq 0 ]; then
    echo "Registering Jupyter kernel 'cnns'..."
    "$VENV_PYTHON" -m ipykernel install --user --name cnns --display-name "Python (cnns .venv)"
fi

echo ""
echo "Setup complete."
echo "Open convolutional-neural-networks.ipynb and pick the 'Python (cnns .venv)' kernel"
