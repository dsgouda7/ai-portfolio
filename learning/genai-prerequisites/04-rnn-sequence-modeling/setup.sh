...
# Jupyter kernel named "rnn-sequence-modeling" pointing at that venv. The notebook's
...
if [ "$SKIP_KERNEL" -eq 0 ]; then
    echo "Registering Jupyter kernel 'rnn-sequence-modeling'..."
    "$VENV_PYTHON" -m ipykernel install --user --name rnn-sequence-modeling --display-name "Python (rnn-sequence-modeling .venv)"
fi

echo ""
echo "Setup complete."
echo "Open rnn-sequence-modeling.ipynb and pick the 'Python (rnn-sequence-modeling .venv)' kernel"
