...
# Jupyter kernel named "tokenization" pointing at that venv. The notebook's
...
if [ "$SKIP_KERNEL" -eq 0 ]; then
    echo "Registering Jupyter kernel 'tokenization'..."
    "$VENV_PYTHON" -m ipykernel install --user --name tokenization --display-name "Python (tokenization .venv)"
fi

echo ""
echo "Setup complete."
echo "Open tokenization-and-embeddings.ipynb and pick the 'Python (tokenization .venv)' kernel"
