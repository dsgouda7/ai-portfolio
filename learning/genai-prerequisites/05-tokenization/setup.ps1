...
.SYNOPSIS
    ...
    tokenization-and-embeddings.ipynb.
...
.DESCRIPTION
    ...
    Jupyter kernel named "tokenization" pointing at that venv. Select the
    "Python (tokenization .venv)" kernel in VS Code to use it.
...
if (-not $SkipKernel) {
    Write-Host "Registering Jupyter kernel 'tokenization'..."
    & $VenvPython -m ipykernel install --user --name tokenization --display-name "Python (tokenization .venv)"
}

Write-Host "`nSetup complete." -ForegroundColor Green
Write-Host "Open tokenization-and-embeddings.ipynb and pick the 'Python (tokenization .venv)' kernel."
