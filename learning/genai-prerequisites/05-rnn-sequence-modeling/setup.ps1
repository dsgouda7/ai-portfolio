...
.DESCRIPTION
    ...
    Jupyter kernel named "rnn-sequence-modeling" pointing at that venv. Select the
    "Python (rnn-sequence-modeling .venv)" kernel in VS Code to use it.
...
if (-not $SkipKernel) {
    Write-Host "Registering Jupyter kernel 'rnn-sequence-modeling'..."
    & $VenvPython -m ipykernel install --user --name rnn-sequence-modeling --display-name "Python (rnn-sequence-modeling .venv)"
}

Write-Host "`nSetup complete." -ForegroundColor Green
Write-Host "Open rnn-sequence-modeling.ipynb and pick the 'Python (rnn-sequence-modeling .venv)' kernel."
