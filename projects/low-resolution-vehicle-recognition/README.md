# RoadID: Low-Resolution Multi-Frame Vehicle Recognition
...
Open `http://127.0.0.1:5000`, select **Deterministic RoadID replay**, and start a run. The replay
...
RoadID treats the **track**, not the frame, as the unit of evidence:
...
RoadID will be a locally hosted Flask application where a user can:
...
RoadID deliberately starts from models trained for broader visual tasks, then adapts them to fine-grained vehicles.
...
- Apache-2.0 model license.
- The ImageNet classifier head will be replaced with RoadID's hierarchical body/make/model heads.
...
No single dataset supplies realistic camera tracks, exact make/model labels, and unrestricted redistribution. RoadID therefore uses explicit dataset adapters and records source terms in every manifest.
...
Each provider record stores attribution, terms URL, camera ID, refresh expectation, location precision policy, and whether frame caching is permitted. RoadID does not scrape Google Maps, arbitrary webcam directories, or web pages.
...
RoadID evaluates the full product, not only frame classification.
...
RoadID is designed for vehicle-category research, not person or owner identification.
