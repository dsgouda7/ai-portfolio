...
image, compares generated predictions with iNaturalist community identifications, and trains a
feed-specific label corrector from data received after the deployed model's watermark.
...
- compares the static SpeciesNet baseline with the currently deployed corrector;
- shows confidence per frame, but never treats confidence as accuracy;
...
| Deployed prediction | Latest WildScope corrector | Version evaluated on unseen post-watermark data |
| Training batch | Eligible observations newer than the deployed watermark | Entire input to the next corrector version |
...
first lifecycle-compatible corrector. After that model is written, normal post-watermark rules apply
...
### Feed-specific label corrector

The trained WildScope component is deliberately small:

```text
SpeciesNet taxonomy label -> iNaturalist scientific-name target
```

It learns categorical correction counts for one protected-area feed. It does **not** fine-tune
SpeciesNet's detector, image encoder, or classifier weights. Calling it a label corrector keeps the
demonstration computationally cheap and the learned behavior inspectable.
...
Confidence is model certainty for one prediction. It is displayed per frame and is not averaged into
an accuracy claim.
...
| Use a categorical corrector instead of image-model fine-tuning | The available per-feed batches are too small and imbalanced for a defensible image-model fine-tune. |
...
4. obtained identification, SpeciesNet output, and deployed-corrector output.
...
src/wildscope/inference.py         SpeciesNet runner, matching, and label corrector
...
static/js/training.js              training-batch and lineage interactions
...
Open `http://127.0.0.1:5000`. The first SpeciesNet run downloads its official detector and
classifier weights. Subsequent runs reuse cached models and images under ignored
`artifacts/runtime/`.
...
- The corrector learns label mappings, not visual features.
