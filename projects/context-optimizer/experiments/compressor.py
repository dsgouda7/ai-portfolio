...
**Guidelines:**
- Extract main entities (people, systems, concepts, error codes)
- Preserve key facts, relationships, and causality
- Keep technical details (metrics, timestamps, identifiers)
- Remove redundant phrasing and filler
- Target: ~50 tokens or less
...
**Output Format (JSON):**
{{
  "summary": "Dense 1-2 sentence summary preserving entities and key facts",
...
def compress_corpus_rolling(
    ...
    """
    Compress a large corpus using a rolling window strategy.

    Process:
    1. Accumulate lines until threshold reached
    2. Compress the accumulated chunk
    3. Move to next chunk (no overlapping context)
    ...
    print(f"[Compressor] Threshold: {chunk_size_threshold} tokens, Batch size: {compression_batch_size}")
