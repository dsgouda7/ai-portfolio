"""Rewrite tree_index.py with N-level configurable depth support."""

import pathlib

TARGET = pathlib.Path(__file__).parent.parent / "src" / "tree_index.py"
...
class TreeIndex:
    ...
    def __init__(
        ...
        counts = "  ".join(
            f"L{lvl}={self._levels[lvl].count()}"
            for lvl in range(1, self._depth + 1)
        )
    ...
    def build_from_chunks(
        ...
        if l1.count() == 0:
            ...
            print(f"{_pfx}[TreeIndex] L1 already populated ({l1.count()} entries), skipping")
        ...
        for lvl in range(2, self._depth + 1):
            ...
            if curr.count() > 0:
                print(f"{_pfx}[TreeIndex] L{lvl} already populated ({curr.count()} entries), skipping")
                existing  = curr.get(include=["documents"])
                prev_ids  = existing["ids"]
                prev_docs = existing["documents"]
                continue

            n_clusters = max(1, (len(prev_ids) + cluster_size - 1) // cluster_size)
            print(
                f"{_pfx}[TreeIndex] Building {n_clusters} L{lvl} super-summaries "
                f"(cluster_size={cluster_size}, input={len(prev_ids)} L{lvl-1} entries) ..."
            )
            ...
            for ci in range(n_clusters):
                ...
                if llm is not None:
                    ...
                    try:
                        ...
                        print(f"{_pfx}[TreeIndex] WARNING L{lvl} cluster {ci} error: {exc}")
...
print(f"Written {len(NEW_CONTENT)} chars to {TARGET.name} — syntax OK")
