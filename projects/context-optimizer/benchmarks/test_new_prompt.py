...
print("=" * 60)
for label, text in [("Old triples", old_triple), ("Prose", old_prose), ("Semantic core (new)", new_core)]:
    toks = _estimate_tokens(text)
    filler = sum(1 for w in text.split() if w.lower() in {"the","a","an","of","in","at","by","for","with","from","to","and","but","or","is","was","were","has","had","be","been","will","his","her","their","it","its","he","she","they","this","that"})
    signal = len(text.split()) - filler
    print(f"\n  {label}")
    print(f"    {text[:100]}...")
    print(f"    tokens={toks}  signal_words={signal}  filler={filler}  signal_ratio={signal/max(1,len(text.split())):.0%}")

print()
print("In a 300-token budget:")
print(f"  Old triples: {300 // _estimate_tokens(old_triple) if _estimate_tokens(old_triple) else 'N/A'}x  -> {_estimate_tokens(old_triple)} tok used, ~{300 - _estimate_tokens(old_triple)} remaining")
print(f"  Prose      : {_estimate_tokens(old_prose)} tokens used, ~{300 - _estimate_tokens(old_prose)} remaining")
print(f"  Core (new) : {_estimate_tokens(new_core)} tokens used, ~{300 - _estimate_tokens(new_core)} remaining  <- {300 // max(1, _estimate_tokens(new_core))}x more facts fit")
