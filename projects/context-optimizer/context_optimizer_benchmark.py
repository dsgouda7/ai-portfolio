...
def load_logs(log_file: str | None, max_log_lines: int = 6000) -> list[str]:
    """Load logs from a text file, else fall back to the in-memory mock corpus."""
    if not log_file:
        return MOCK_LOG_CACHE
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--pipeline",
        choices=["both", "raw", "optimized"],
        default=os.getenv("PIPELINE_MODE", "both"),
        help="Run both pipelines, only raw baseline, or only optimized flow.",
    )
    parser.add_argument(
        "--log-file",
    ...
    parser.add_argument(
        "--max-log-lines",
        type=int,
        default=int(os.getenv("MAX_LOG_LINES", "6000")),
        help="Maximum number of log lines to load from --log-file.",
    )
    parser.add_argument(
        "--metrics-json",
